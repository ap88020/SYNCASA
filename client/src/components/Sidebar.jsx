  ...
  return (
    <div
      className={`fixed sm:static z-20
                  ...
                  bg-white dark:bg-surface-dark border-t sm:border-t-0 sm:border-r border-gray-200 dark:border-white
      ...
      <div className="flex sm:flex-col w-full h-full justify-around sm:justify-start text-sm font-medium text-gray-600 dark:text-gray-300">
        {navItmes.map(({ to, label, Icon }) => (
          <NavLink
            ...
            className={({ isActive }) =>
              `flex flex-col sm:flex-row items-center justify-center sm:justify-start gap-1 sm:gap-3 px-2 sm:px-4 py-1.5 sm:py-2.5 rounded-none sm:rounded dark:shadow dark:shadow-white ${
                isActive
                  ? "bg-gradient-to-r from-[#9234EA] to-pink-500 text-white sm:bg-transparent sm:text-white"
                  : ""
              }`
            }
          >
              ...
              <>
                <Icon
                  className={`w-5 h-5 sm:w-4 sm:h-4 ${
                    isActive ? "text-white sm:text-white font-extrabold" : ""
                  }`}
                />
